-- Initial base schema for a fresh database.
--
-- This is the frozen "migration zero" that creates every base table before any
-- of the incremental ALTER migrations run. It is a hand-maintained, idempotent
-- copy of lib/models/schema.gisila.up.sql (the build_runner snapshot, which is
-- only a reference for `migrate diff` and is NOT applied directly).
--
-- Every statement is guarded (IF NOT EXISTS / DROP CONSTRAINT IF EXISTS) so the
-- migration is a safe no-op on databases that were provisioned before this file
-- existed and already have these objects.
--
-- When models change, do NOT edit this file: generate a new incremental
-- migration in this directory via `dart run gisila_orm:migrate diff`.
...
-- Foreign keys. DROP ... IF EXISTS makes the ADD idempotent (Postgres has no
...
CREATE INDEX IF NOT EXISTS "idx_managed_services_service_type" ON "managed_services" ("service_type");
