...
extension AuthApiGisilaDoc on AuthApi {
  ...
  void attachToApp(
    ...
    spec.putSchema('RegisterForm', <String, Object?>{
      'type': 'object',
    ...
    spec.putTag('Auth');
    {
      final basePath = '$prefix/auth/register';
      final openApiPath = '$prefix/auth/register';
      final RouteConfig __cfg = RouteConfig.empty
          .merge(const RouteConfig(isPublic: true))
          .merge(const RouteConfig(
              rateLimit: const RateLimitConfig(requestsPerMinute: 20)));
      router.post(
          basePath,
          gisilaRoute(
            app: app,
            ...
            handler: (RequestContext ctx) async {
              try {
                ...
                final form = await bindForm(request, RegisterForm.new);
                final auth = ctx.service<AuthService>();
                final result = await this.register(form, auth);
          ...
          Operation(
            ...
            requestBody: RequestBody(required: true, content: {
              'application/json': MediaType(schema: <String, Object?>{
                r'$ref': '#/components/schemas/RegisterForm'
              })
            }),
