...
Future<void> _mailEnsureStack() async {
  ...
  const postconf = <String>[
    ...
    'milter_protocol=6',
    // Use a Unix domain socket so there is no IPv6/localhost ambiguity and
    // no need for opendkim to bind a TCP port.
    // Inside the chroot (port-25 smtpd) Postfix sees "private/opendkim.sock".
...
''';
  ...
  //
  // Socket strategy: use a Unix domain socket inside Postfix's spool directory
  // so both chroot'd and non-chroot'd Postfix services can reach the milter
  // without any TCP stack or IPv6/localhost resolution issues.
  // Postfix references it as "unix:private/opendkim.sock" (chroot-relative).
  const _opendkimSock = '/var/spool/postfix/private/opendkim.sock';
  const opendkimConf = '''
...
''';
  ...
  // /run/opendkim lives on tmpfs; create it here AND install a tmpfiles.d
  // snippet so systemd recreates it on every boot before opendkim starts.
  ...
  // Allow postfix (and opendkim itself) to create/use the Unix socket inside
  // /var/spool/postfix/private. opendkim runs as opendkim; postfix as postfix.
  // UMask 007 in opendkim.conf creates the socket 0660. Add opendkim to the
  // postfix group so the socket is accessible to both sides.
  await _sudo('usermod', ['-aG', 'postfix', 'opendkim'], failOk: true);
