...
Future<void> _mailEnsureStack() async {
  ...
  const postconf = <String>[
    ...
    // TLS — snakeoil by default so STARTTLS/SMTPS work out of the box.
  ...
  await _sudo('postconf',
      ['-M', 'submission/inet=submission inet n - y - - smtpd']);
  ...
  await _sudo('postconf', ['-M', 'smtps/inet=smtps inet n - y - - smtpd']);
  for (final p in <String>[
  ...
  // IMAP + POP3 over TLS (snakeoil cert).
  const dovecotConf = r'''
...
# TLS — snakeoil by default; point these at a real cert in production.
ssl = yes
